...
│      ├─ SQLite audit DB (in-container file; JSONL fallback)                         │
...
These are the **exact versions resolved in the lockfile** (`pnpm-lock.yaml`), verified by a clean `pnpm install`. Where this differs from older prose, **this table wins** (see §9, Drift notes).
...
- `frontend`: builds `./frontend`, sets `VITE_API_BASE=http://localhost:8000`, publishes `5173:5173`, `depends_on: backend`.
...
- **Audit / run store:** SQLite via better-sqlite3 — a single file inside the backend container. Synchronous driver, so DB calls are short and ordered. **Append-only** for audit events: there is no delete path (deleting audit history is a rubric hard-fail). A **JSONL fallback** activates if SQLite is unavailable, so audit durability never blocks the demo.
...
- **Persistence across container restarts** is not configured by default (the DB file lives in the container's writable layer). For a long-lived deployment, add a named volume for the DB path; for the hackathon, run durability within a session is sufficient and audit history is also mirrorable to JSONL.
