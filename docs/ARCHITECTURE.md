...
`apps/dashboard` is the single operational UI. It is a Next.js dashboard that renders service desk tickets, runs, approvals, audit evidence, activity state, memory visibility, observability status, and backend health from Hono-owned contracts.

The Hono backend is the source of truth for Phoenix access, SSH execution, LLM proposal flow, deterministic safety checks, technician approvals, audit writes, SSE events, activity drafting and submission, memory rules, and observability rules. The dashboard calls typed HTTP and SSE endpoints only; it does not own those rules and must not call Phoenix, SSH, model, safety, store, or audit internals directly.

## 1. Stack decision (opinionated, final)

| Concern     | Choice                                                                               | Why (and what we rejected)                                                                                                                                                                                                                                                                                         |
| ----------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Runtime     | **Node 22 + Bun (workspaces) + tsx**                                                 | Native fetch/Web Streams; Bun workspace install; `tsx` = zero-build dev.                                                                                                                                                                                                                                            |
...
| Model/agent | **Vercel AI SDK v5**                                                                 | Native tool calling, `generateObject`/`Output.object` structured output, `stopWhen`, `ToolLoopAgent`, HITL approval. Rejected **LangChain/LangGraph**: heavier abstractions, slower to debug at 3am, and we want the _backend_ to own the loop, not a framework.                                                   |
...
| Persistence | **SQLite via better-sqlite3** for runs/audit; **Postgres + pgvector** for RAG memory | SQLite stays synchronous and zero-server for operational data. Postgres holds redacted solution embeddings for retrieval and continual learning. JSONL fallback if SQLite is unavailable in mock mode.                                                                                                             |
...
**One orchestrated loop with specialist _roles_.** The "agents" (Diagnostics, Fix Planner,
Validator, Activity Writer) are distinct prompts + output schemas invoked by one orchestrator -
not independent processes. This _feels_ agentic in the UI but stays linear and debuggable.
...
backend/
  ...
  src/
    ...
    ai/
      ...
      prompts.ts             # system prompts per role (see section 8)
...
**Invariant:** the model's proposed command and the _executed_ command pass through the safety
...
## 5. Why audit persistence is part of the _product_, not plumbing
...
activity** (rubric B: no invented actions). So the audit store is a _feature_: it's what the judge
...
| **`problem_analyzer`** (diagnostics) | ticket symptom, observations so far           | `DiagnosticProposal` (section 8) incl. **ranked hypotheses + evidence** | One command, prefer read-only, explain expected signal. Surfaces the ranked hypothesis list the technician picks from (brief's "what great looks like"). Scope = local Linux services only. |
...
| **SafetyReviewer**                   | a command string                              | `RiskAssessment`                                                 | **Deterministic policy first**; optional LLM second opinion. Can only _raise_ concern, never override a deterministic block.                                                                |
...
| **Validator**                        | fix + post-fix observations                   | `ValidationResult` (section 8)                                          | Distinguishes `VERIFIED_FIXED` vs `LIKELY_FIXED`; **proposes a persistence/reboot check** (graded).                                                                                         |
...
- **`problem_analyzer` (diagnostics):** "Every incident is a **local Linux service problem** - a
  ...
  `free -m`, `tail`/`grep` of a _specific_ log. Narrow to a root cause; don't fish."
- **Fix Planner:** "Output one `FixProposal` that addresses the _root cause_, not the symptom.
...
- **Validator:** "Output a `ValidationResult`. Only say `VERIFIED_FIXED` if an observation
  concretely proves the customer benefit is restored _and_ would survive a restart. Otherwise
...
```json
{
  ...
  "hypotheses": [
    {
      "rootCause": "status-api systemd unit failed on a stale PID/port bind",
      "confidence": 0.6,
      "evidence": [
        "ticket: 'API intermittently unavailable'",
        "service-class symptom, not network"
      ]
    },
    {
      "rootCause": "disk full preventing the service from writing",
      "confidence": 0.25,
      "evidence": [
        "intermittent failures can indicate transient resource exhaustion"
      ]
    }
  ],
  ...
  "riskNotes": "Read-only systemd query, bounded output, no secrets.",
  "isReadOnly": true
}
```
...
```json
{
  ...
  "evidence": [
    "curl -I localhost:8080 => HTTP/1.1 200",
    "systemctl is-active status-api => active"
  ],
...
These extend section 3/section 4/section 8/section 10; full detail in [RELIABILITY.md](./RELIABILITY.md) + [AGENT_PIPELINE.md](./AGENT_PIPELINE.md).
...
**SSH / reliability hardening (counters the documented top failure modes - see RELIABILITY.md section 1,section 4):**
