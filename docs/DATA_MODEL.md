# Data Model — Service Desk Autopilot
...
Every meaningful side-effect emits one audit event *and* one SSE event with the same `type` — so the live timeline and the permanent record can never diverge.
...
*Companions: [ARCHITECTURE.md §6](ARCHITECTURE.md) (the original schema sketch) · [SAFETY_POLICY.md §6–7](SAFETY_POLICY.md) (redaction + the mandatory audit record) · [API.md](API.md) (how this surfaces over HTTP/SSE).*
