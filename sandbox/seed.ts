...
import { parseSandboxCaseCount, scenariosForHost, SCENARIOS } from './scenarios/registry.ts';
...
function run(command: string, args: string[], options: { allowFailure?: boolean; quiet?: boolean } = {}) {
  ...
  const result = spawnSync(command, args, { cwd: repoRoot, encoding: 'utf8', stdio: options.quiet ? 'pipe' : 'inherit' });
...
function runScenarioContainer(scenario: Scenario) {
  ...
  const cgroupMount = privileged ? '/sys/fs/cgroup:/sys/fs/cgroup:rw' : '/sys/fs/cgroup:/sys/fs/cgroup:ro';
  const args = [
    'run',
    '-d',
    '--name',
...
function waitForSystemd(scenario: Scenario) {
  ...
  for (let attempt = 0; attempt < 30; attempt += 1) {
    const result = run('docker', ['exec', name, 'bash', '-lc', 'systemctl is-active --quiet ssh && systemctl is-system-running'], {
      allowFailure: true,
      quiet: true,
    });
...
function printSummary(scenarios: Scenario[]) {
  ...
  console.log('SANDBOX_SSH_HOST=host.docker.internal   # backend containers -> host-mapped fake VMs');
  ...
  for (const scenario of scenarios) {
    console.log(`${scenario.ticket.id}      ${scenario.system.ip}:${scenario.system.port}     ${scenario.archetype}`);
  }
}

function removeAllScenarioContainers(): boolean {
  const names = dockerOutput(['ps', '-a', '--filter', `label=${labelKey}=${labelValue}`, '--format', '{{.Names}}']);
...
function up() {
  if (!existsSync(publicKeyPath)) {
    throw new Error(`Missing ${publicKeyPath}. Reuse keys/bench_incident_key.pub; do not generate or commit a new key.`);
  }
