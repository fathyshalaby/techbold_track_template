...
describe('sandbox scenario registry', () => {
  ...
  it('uses unique Phoenix-like ticket and customer ids', () => {
    ...
    expect(new Set(SCENARIOS.map((scenario) => scenario.ticket.customer_id)).size).toBe(SCENARIOS.length);
    expect(SCENARIOS.every((scenario) => scenario.ticket.id >= 7101 && scenario.ticket.id <= 7199)).toBe(true);
    expect(SCENARIOS.every((scenario) => scenario.ticket.customer_id >= 5101 && scenario.ticket.customer_id <= 5199)).toBe(true);
  });

  it('targets localhost-only SSH ports', () => {
    const systems = scenarioCustomerSystems();
    expect(Object.values(systems).every((customerSystem) => customerSystem.system.ip === '127.0.0.1')).toBe(true);
    expect(Object.values(systems).map((customerSystem) => customerSystem.system.port).sort()).toEqual([2201, 2202, 2203, 2204, 2205]);
  });

  it('keeps ticket descriptions symptom-only', () => {
    const forbidden = /(root cause|chown|chmod|grant|sequence|\/etc\/hosts|Environment=|enable --now|127\.0\.0\.2)/i;
