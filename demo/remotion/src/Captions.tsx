...
export const Captions: React.FC<{ cues: Cue[] }> = ({ cues }) => {
  ...
  const opacity = interpolate(local, [0, 8, span - 8, span], [0, 1, 1, 0], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const x = interpolate(local, [0, 10], [-18, 0], { extrapolateRight: "clamp" });

  return (
    <div style={{ position: "absolute", left: 44, bottom: 44, opacity, transform: `translateX(${x}px)`, fontFamily }}>
      <div
        style={{
