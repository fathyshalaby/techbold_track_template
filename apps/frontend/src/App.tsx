...
export default function App() {
    ...
    (runId: string) => {
      ...
      const handleEvent = (m: MessageEvent) => {
        try {
          ...
          setEvents((prev) => [
            ...prev,
            { id: `${ev.ts}-${ev.type}-${prev.length}`, type: ev.type, ts: ev.ts },
          ]);
        } catch {
          return;
        }
  ...
  const startRun = async (ticket: Ticket) => {
    ...
    setDraft(null);
    try {
      const created = await api<{ runId: string; status: string; customerSystem: CustomerSystem }>(
        "/api/runs",
        {
          method: "POST",
  ...
  const draftActivity = () =>
    act(async () => {
      const d = await api<ActivityDraft>(`/api/runs/${run!.runId}/activity/draft`, {
        method: "POST",
      });
      setDraft(d);
    });
  const submitActivity = () =>
    act(() => {
      ...
      const body = d
        ...
        : {};
      return api(`/api/runs/${run!.runId}/activity/submit`, {
        method: "POST",
  ...
  return (
    <main style={wrap}>
      ...
      {error && (
        <div style={{ ...card, borderColor: "#cf222e", background: "#ffebe9", color: "#cf222e" }}>
          {error}
        </div>
      )}
      ...
      {!run && (
        <section>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <h2>Tickets</h2>
            <button type="button" style={btn} onClick={() => void loadTickets()} disabled={busy}>
              Refresh
            </button>
          </div>
          {tickets.length === 0 && (
            <p style={{ color: "#666" }}>No tickets (or backend unreachable).</p>
          )}
          {tickets.map((t) => (
      ...
      {run && (
        <section>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button type="button" style={btn} onClick={closeRun}>
              Back to tickets
            </button>
          </div>
          <div style={{ color: "#666", margin: "6px 0 16px" }}>
            ...
            {system && (
              <>
                ...
                · target{" "}
                <code>
                  {system.username}@{system.ip}:{system.port}
                </code>{" "}
                ({system.os})
              </>
            )}
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            {run.phase === "WAITING_FOR_ACTIVITY_REVIEW" && (
              <button type="button" style={btn} onClick={draftActivity} disabled={busy}>
                Draft activity
              </button>
            )}
            <button
              type="button"
              style={{ ...btn, color: "#cf222e" }}
              onClick={abort}
              disabled={busy}
            >
              Abort
        ...
        {timeline.map((e) => {
          ...
          return (
            <div key={e.id} style={{ padding: "3px 0", borderBottom: "1px solid #f0f0f0" }}>
              <span style={{ color: "#666" }}>{new Date(e.ts).toLocaleTimeString()}</span>{" "}
              <span style={{ color: ACTOR_COLORS[e.actor] ?? "#666", fontWeight: 600 }}>
                {e.actor}
              </span>{" "}
              <span>{e.type}</span>
...
function ApprovalCard(props: {
  ...
  return (
    <div style={{ ...card, borderColor: risk, borderWidth: 2 }}>
      ...
      <p style={{ margin: "4px 0" }}>
        <strong>Expected:</strong> {a.expected_signal}
      </p>
      {a.safety_notes && (
        <p style={{ margin: "4px 0", color: "#bc4c00" }}>
          <strong>Safety:</strong> {a.safety_notes}
        </p>
      )}
      <label
        htmlFor="approval-command"
        style={{ display: "block", fontSize: 13, color: "#666", marginTop: 8 }}
      >
        Command (editable before approving):
      </label>
      <textarea
        id="approval-command"
        value={editCmd}
      ...
      >
        <button
          type="button"
          style={{ ...btn, background: "#1a7f37", color: "#fff", borderColor: "#1a7f37" }}
          onClick={onApprove}
          disabled={busy}
        >
          Approve &amp; run
    ...
    <div style={card}>
      ...
      {fields.map(([k, label]) => (
        <div key={k} style={{ marginBottom: 8 }}>
          <label htmlFor={`draft-${k}`} style={{ display: "block", fontSize: 13, color: "#666" }}>
            {label}
          </label>
          <textarea
            id={`draft-${k}`}
