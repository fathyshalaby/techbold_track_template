...
class PhoenixClient:
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> httpx.Response:
        ...
        for attempt in range(3):
            ...
            if resp.status_code >= 500:
                last_err = ERPError(
                    f"{resp.status_code} from Phoenix {path}", resp.status_code
                )
                time.sleep(0.4 * (attempt + 1))
                continue
            ...
            if resp.status_code >= 400:
                raise ERPError(
                    f"{resp.status_code} from Phoenix {path}: {resp.text[:300]}",
                    resp.status_code,
                )
            return resp
    ...
    def list_tickets(
        self, status: str | None = None, priority: str | None = None, sort: str = "date"
    ) -> list[dict]:
        params = {
            k: v
            for k, v in (("status", status), ("priority", priority), ("sort", sort))
            if v
        }
