...
TOOLS: list[dict] = [
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {"type": "string", "description": "One shell command. No secrets. No chaining unless justified."},
                    "purpose": {"type": "string", "enum": ["diagnose", "fix", "validate"]},
                    "rationale": {"type": "string", "description": "Why this command, in one or two sentences."},
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "root_cause": {"type": "string", "description": "The technical root cause, not the symptom."},
                    "fixed": {"type": "boolean"},
                    "validation_result": {"type": "string", "description": "Concrete proof, or why still blocked."},
                },
...
def initial_messages(ticket: dict, system: dict) -> list[dict]:
    ...
    runbook = knowledge.select_runbooks(f"{ticket.get('title', '')} {ticket.get('description', '')}")
    if runbook:
        messages.append({"role": "system", "content": f"Relevant runbook for this symptom class:\n{runbook}"})
    return messages
...
def next_action(messages: list[dict]) -> dict:
    ...
    if tool_calls:
        ...
        return {
            "type": tc.function.name if tc.function.name in ("run_command", "conclude") else "message",
            "args": args,
            "tool_call_id": tc.id,
            "assistant_message": assistant,
        }
    return {"type": "message", "content": msg.content or "", "assistant_message": assistant}
