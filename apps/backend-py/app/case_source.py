...
_LOCAL_HOSTS = {"localhost", "127.0.0.1", "0.0.0.0", "::1", "host.docker.internal", "phoenix-mock"}
_selected: CaseSourceSelection = "sandbox_cases" if settings.sandbox_case_count > 0 else "real_erp"
...
def set_case_source(source: str) -> CaseSourceSelection:
    ...
    if source == "sandbox_cases" and not sandbox_available():
        raise ValueError("sandbox cases are not seeded; set SANDBOX_CASE_COUNT > 0 and restart compose")
