...
def _shared_dir() -> Path:
    candidates = [
        ...
        Path(__file__).resolve().parent / "shared",        # bundled: apps/backend-py/app/shared
        Path(__file__).resolve().parents[3] / "shared",    # monorepo root /shared (if present)
    ]
    for c in candidates:
...
_OBFUSCATION = re.compile(r"\$\(|\$\{|`")  # subshell / unresolved var expansion / backtick
...
def classify(command: str) -> dict:
    ...
    if _OBFUSCATION.search(cmd):
        return {"classification": "needs_review", "risk": "needs_review", "matched_rule": "obfuscated", "reason": None}
...
_REDACTION_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"-----BEGIN[\s\S]*?PRIVATE KEY-----[\s\S]*?(?:-----END[^-]*PRIVATE KEY-----|$)"), "«redacted»"),
    ...
    (re.compile(r"(?i)(postgres(?:ql)?|mysql|mongodb|redis)://[^@\s]+@[^\s]*"), r"\1://«redacted»"),
    ...
    (re.compile(r'(?i)("[A-Za-z0-9_-]*(?:token|secret|passwd|password|api[_-]?key|key|authorization|auth|credential)[A-Za-z0-9_-]*"\s*:\s*)"[^"]*"'), r'\1"«redacted»"'),
    # env-style SECRET=... (negative lookahead avoids double-redacting)
    (re.compile(r"\b([A-Za-z_]*(?:SECRET|TOKEN|KEY|PASS|PASSWORD|CREDENTIAL|secret|token|key|pass|password|credential)[A-Za-z_0-9]*\s*=\s*)(?!«redacted»)\S+"), r"\1«redacted»"),
    # custom secret headers (X-Phoenix-Token: ...)
    (re.compile(r"(?i)([Xx]-[A-Za-z0-9-]*(?:token|key|secret|auth)[A-Za-z0-9-]*\s*:\s*)\S+"), r"\1«redacted»"),
    ...
    (re.compile(r"\b(?:gh[pousr]_[A-Za-z0-9]{20,}|xox[baprs]-[A-Za-z0-9-]{10,}|sk-[A-Za-z0-9]{20,}|sk_live_[A-Za-z0-9]{16,}|rk_live_[A-Za-z0-9]{16,}|glpat-[A-Za-z0-9_-]{16,}|AIza[A-Za-z0-9_-]{16,}|ya29\.[A-Za-z0-9_-]{20,})"), "«redacted»"),
