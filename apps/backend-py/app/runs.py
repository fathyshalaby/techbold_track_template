...
_MAX_AUTO_ADVANCE = (
    6  # guard against runaway auto-advance (safety-blocked / no-tool-call loops)
)
    ...
    while True:
        ...
        if run._auto >= _MAX_AUTO_ADVANCE:
            ...
            run.log(
                "system",
                "error",
                note="too many automatic advances without a human-approvable step",
            )
            return
        run._auto += 1
        ...
        if atype == "conclude":
            ...
            run.log(
                "agent",
                "validated",
                note=safety.redact(run.conclusion.get("validation_result", "")),
            )
            if run._ssh:
                run._ssh.close()
            run.emit()
        ...
        run.messages.append(
            {
                "role": "user",
                "content": "Use the run_command tool to propose ONE command, or call conclude.",
            }
        )
...
def _execute(run: Run, step: Step, command: str, auto: bool) -> None:
    ...
    try:
        ...
        run.messages.append(
            agent.tool_result_message(step.tool_call_id, f"SSH ERROR: {exc}")
        )
        run.emit()
        return
    step.ran_at = _now()
    ...
    run.messages.append(
        agent.tool_result_message(step.tool_call_id, tool_content[:8000])
    )
    run.emit()
...
def approve_step(
    run_id: str, step_id: str, edited_command: Optional[str] = None
) -> Run:
    run = _get(run_id)
    step = _find_step(run, step_id)
    if (
        step is None
        or step.status != "pending_approval"
        or run.pending_step_id != step_id
    ):
    ...
    if verdict["classification"] == "blocked":
        ...
        run.messages.append(
            agent.tool_result_message(
                step.tool_call_id,
                f"BLOCKED by safety layer: {verdict['reason']}. Propose a safer approach.",
            )
        )
        run.emit()
        threading.Thread(target=_drive, args=(run,), daemon=True).start()
...
def submit_activity(run_id: str, draft: dict) -> dict:
    ...
    run.log(
        "technician",
        "activity_submitted",
        note=f"activity created for ticket {run.ticket_id}",
    )
    if run._ssh:
        run._ssh.close()
