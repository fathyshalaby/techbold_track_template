...
_RUNBOOKS = [
    {
        "id": "systemd-services",
        "keywords": ["service", "unit", "systemctl", "start", "won't start", "crash", "failed", "masked",
                     "daemon", "restart", "timer", "enable", "enabled", "disabled", "boot", "override",
                     "drop-in", "environment", "stale"],
        "digest": (
    ...
    },
    {
        ...
        "keywords": ["port", "bind", "listen", "firewall", "ufw", "dns", "resolve", "refused", "nginx",
                     "apache", "502", "504", "tls", "ssl", "cert", "https", "http", "proxy", "curl",
                     "connection", "timeout", "unreachable", "hosts", "/etc/hosts", "resolution",
                     "upstream", "partner", "dependency", "reach"],
        "digest": (
    ...
    },
    {
        ...
        "keywords": ["disk", "inode", "no space", "space", "full", "read-only", "readonly", "log", "oom",
                     "memory", "swap", "cpu", "load", "fd", "file descriptor", "too many open", "137", "killed"],
        "digest": (
    ...
    },
    {
        ...
        "keywords": ["postgres", "postgresql", "mysql", "mariadb", "database", "db", "sql", "permission",
                     "denied", "chown", "chmod", "owner", "sudo", "cron", "crontab", "schedule", "apparmor",
                     "selinux", "config", "access", "5432", "3306", "socket", "grant", "revoke", "privilege",
                     "sequence", "insert", "write", "role", "upload", "writable"],
        "digest": (
...
def select_runbooks(symptom_text: str, max_n: int = 2) -> str:
    ...
    scored = [
        (sum(1 for k in r["keywords"] if k in t), r)
        for r in _RUNBOOKS
    ]
    picked = [r for score, r in sorted(scored, key=lambda x: -x[0]) if score > 0][:max_n]
