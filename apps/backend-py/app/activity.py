...
def generate(run) -> dict:
    ...
    messages = [
        {
            "role": "system",
            "content": "You write clean, accurate IT incident documentation. Output strict JSON.",
        },
    ...
    try:
        msg = llm.chat(
            messages, response_format={"type": "json_object"}, temperature=0.2
        )
    ...
    if not draft["description"].strip():
        draft["description"] = (
            draft["summary"] + " " + draft["validation_result"]
        ).strip()
    return draft
...
def to_payload(run, draft: dict) -> dict:
    ...
    return {
        ...
        "description": safety.redact(draft.get("description", ""))
        or safety.redact(draft.get("summary", "")),
