    ...
    def _candidate_keys(self) -> list[str]:
        ...
        try:
            ...
            for pattern in patterns:
                for p in sorted(
                    glob.glob(
                        os.path.join(settings.ssh_key_dir, pattern), recursive=True
                    )
                ):
    ...
    def connect(self) -> None:
        ...
        for key_path in self._candidate_keys():
            ...
            try:
                ...
                raise SSHError(
                    f"SSH connect to {self.host}:{self.port} failed: {exc}"
                ) from exc
            ...
            return
        raise SSHError(
            f"SSH auth to {self.host}:{self.port} failed for all keys [{'; '.join(errors) or 'no keys found'}]"
        )
