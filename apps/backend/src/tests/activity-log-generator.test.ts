...
const SCRIPTED_DRAFT = {
  ...
  commandsSummary:
    "systemctl status nginx (exit 3); fuser -k 80/tcp (exit 0); systemctl start nginx (exit 0)",
...
const SAMPLE_INPUT = {
  auditEvents: [
    { type: "run.started", actor: "system", ts: "2026-06-07T10:00:00Z", payload_json: "{}" },
    {
      ...
      payload_json: '{"command":"systemctl status nginx"}',
    },
  ],
  commandResults: [
    {
      ...
      exitCode: 3,
      stdoutRedacted: "Active: inactive",
      stderrRedacted: "",
    },
...
describe("runActivityLogGenerator", () => {
  ...
  it("throws AgentUnavailableError when the model rejects", async () => {
    ...
    await expect(runActivityLogGenerator(SAMPLE_INPUT, model)).rejects.toBeInstanceOf(
