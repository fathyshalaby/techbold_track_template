...
describe("vertical service-desk slice", () => {
  it("covers run creation, SSE consumption, edited approval execution, validation, and activity submission", async () => {
    ...
    expect(create.body.ticket).toEqual(
      expect.objectContaining({
        id: 1,
        title: "Service unavailable",
      }),
    );
    expect(create.body.customerSystem).toEqual(
      expect.objectContaining({
        ip: "10.0.0.1",
        port: 22,
        username: "azureuser",
    ...
    expect(diagnosticStep.pendingApproval).toEqual(
      expect.objectContaining({
    ...
    expect(diagnosticApprovalRes.body.result).toEqual(
      expect.objectContaining({
    ...
    expect(fixStep.pendingApproval).toEqual(
      expect.objectContaining({
    ...
    expect(fixApprovalRes.body.result).toEqual(
      expect.objectContaining({
    ...
    const finalRun = (await finalRunRes.json()) as {
      status: string;
      phase: string;
      activityDraft: { submitted: number };
    };
