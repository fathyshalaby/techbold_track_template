...
async function seedRunWithApproval(
  status: "PENDING" | "APPROVED" | "REJECTED" | "EXECUTED" | "BLOCKED" = "PENDING",
  riskLevel = RiskLevel.SAFE_READ_ONLY,
) {
...
describe("POST /api/runs/:runId/approvals/:approvalId/approve", () => {
  it("returns 200 with phase OBSERVING when no editedCommand", async () => {
    ...
    const res = await app.request(`/api/runs/${run.id}/approvals/${approval.id}/approve`, {
      method: "POST",
  ...
  it("calls advance with finalCommand = editedCommand when editedCommand is provided", async () => {
    ...
    const advanceSpy = vi
      .spyOn(orchestratorModule, "advance")
      .mockResolvedValueOnce(makeObservingState(run.id));

    await app.request(`/api/runs/${run.id}/approvals/${approval.id}/approve`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    expect(advanceSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        finalCommand: "systemctl status nginx --no-pager",
      }),
    );
  });
  ...
  it("returns 422 with riskLevel when advance returns WAITING_FOR_APPROVAL (blocked command)", async () => {
    ...
    const res = await app.request(`/api/runs/${run.id}/approvals/${approval.id}/approve`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ editedCommand: "rm -rf /" }),
    });
  ...
  it("returns 404 for unknown runId", async () => {
    const res = await app.request("/api/runs/run_nonexistent/approvals/appr_abc/approve", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
  ...
  it("returns 404 for unknown approvalId", async () => {
    ...
    const res = await app.request(`/api/runs/${run.id}/approvals/appr_nonexistent/approve`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
...
describe("POST /api/runs/:runId/approvals/:approvalId/reject", () => {
  it("returns 200 with status TRIAGING when rejected with reason", async () => {
    ...
    const res = await app.request(`/api/runs/${run.id}/approvals/${approval.id}/reject`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("calls advance with command_rejected event and the reason", async () => {
    ...
    const advanceSpy = vi
      .spyOn(orchestratorModule, "advance")
      .mockResolvedValueOnce(makeTriagingState(run.id));

    await app.request(`/api/runs/${run.id}/approvals/${approval.id}/reject`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 404 for unknown runId", async () => {
    const res = await app.request("/api/runs/run_nonexistent/approvals/appr_abc/reject", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 404 for unknown approvalId", async () => {
    ...
    const res = await app.request(`/api/runs/${run.id}/approvals/appr_nonexistent/reject`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ reason: "nope" }),
    });
