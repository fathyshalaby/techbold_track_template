...
describe("Prompt generalization (DIAG-02 / SC2)", () => {
  ...
  it("CUSTOMER_SYSTEM_ANALYZER_SYSTEM_PROMPT does not contain hardcoded incident identifiers", async () => {
    ...
    expect(CUSTOMER_SYSTEM_ANALYZER_SYSTEM_PROMPT).not.toMatch(
      /\b(status-api|vm-01|ticket[_-]?\d+)\b/i,
    );
  });
...
describe("reducer transitions", () => {
  ...
  it("1. TRIAGING + diagnostic_proposal_ready -> WAITING_FOR_APPROVAL with createPendingApproval + emitEvent", async () => {
    ...
    expect(
      result.sideEffects.some(
        (e) =>
          e.type === "emitEvent" &&
          (e as { type: "emitEvent"; runId: string; eventType: string; payload: unknown })
  ...
  it("2. TRIAGING + command_blocked -> TRIAGING with auditCommandBlocked", async () => {
    ...
    expect(
      result.sideEffects.some(
        (e) =>
          e.type === "appendAuditEvent" &&
          (e as { type: "appendAuditEvent"; auditType: string }).auditType === "command.blocked",
      ),
    ).toBe(true);
  });
  ...
  it("3. WAITING_FOR_APPROVAL + command_rejected -> TRIAGING with auditRejected", async () => {
    ...
    expect(
      result.sideEffects.some(
        (e) =>
          e.type === "appendAuditEvent" &&
          (e as { type: "appendAuditEvent"; auditType: string }).auditType === "command.rejected",
      ),
    ).toBe(true);
  });
  ...
  it("4. EXECUTING_COMMAND + command_result -> OBSERVING with appendObservation + command.completed audit", async () => {
    ...
    expect(
      result.sideEffects.some(
        (e) =>
          e.type === "appendAuditEvent" &&
          (e as { type: "appendAuditEvent"; auditType: string }).auditType === "command.completed",
      ),
    ).toBe(true);
  });
  ...
  it("5. OBSERVING + root_cause_found -> PLANNING_FIX", async () => {
    ...
    const result = reduce(state, {
      ...
      hypothesis: "nginx crashed due to OOM",
    });
  ...
  it("9. VALIDATING + validation_complete LIKELY_FIXED -> DRAFTING_ACTIVITY", async () => {
    ...
    const result = reduce(state, {
      type: "validation_complete",
      result: { status: "LIKELY_FIXED", benefitCheck: "ok", persistenceCheck: null, evidence: [] },
    });
    expect(result.nextState.phase).toBe("DRAFTING_ACTIVITY");
  });
  ...
  it("11. Max-steps cap at stepCount=12 -> WAITING_FOR_ACTIVITY_REVIEW with run.steps_capped audit", async () => {
    ...
    expect(
      result.sideEffects.some(
        (e) =>
          e.type === "appendAuditEvent" &&
          (e as { type: "appendAuditEvent"; auditType: string }).auditType === "run.steps_capped",
      ),
    ).toBe(true);
  });
  ...
  it("16. PLANNING_FIX + command_blocked -> TRIAGING with command.blocked audit", async () => {
    ...
    expect(
      result.sideEffects.some(
        (e) =>
          e.type === "appendAuditEvent" &&
          (e as { auditType: string }).auditType === "command.blocked",
      ),
    ).toBe(true);
    expect(
      result.sideEffects.some(
...
describe("orchestrator driver - integration", () => {
  ...
  it("Test 1 - happy path TRIAGING -> WAITING_FOR_APPROVAL", async () => {
    ...
    const db = getDb();
    const approvals = db.all<{ run_id: string; status: string }>(
    ...
    const approvalRow = approvals.find((a) => a.status === "PENDING") as
  ...
  it("Test 8 - re-gate blocks an edited dangerous command: no execution, audited", async () => {
    ...
    const execSpy = vi.fn();
    const spyExecutor = {
      executeApprovedCommand: execSpy,
      runPreflight: vi.fn(),
  ...
  it("Test 9 - redaction at the sink: secrets in output never persist raw", async () => {
    ...
    const leakyExecutor = {
      executeApprovedCommand: vi.fn().mockResolvedValue({
        stdout: "db connect: token=SUPERSECRET123",
        stderr: "",
        exitCode: 0,
        durationMs: 5,
        timedOut: false,
    ...
    const results = db.all<{ stdout_redacted: string }>(
      "SELECT * FROM command_results WHERE run_id = ?",
      [run.id],
    );
  ...
  it("Test 12 - observation records exit code and stderr, not only stdout", async () => {
    ...
    // The info-first preflight is also an "ssh" observation, so target the
    // command-result observation specifically (the one with the exit code).
    const sshObs = db
      .all<{ source: string; content: string }>("SELECT * FROM observations WHERE run_id = ?", [
        run.id,
      ])
      .find((o) => o.source === "ssh" && o.content.includes("exit_code:"));
  ...
  // Info-first: the run gathers read-only preflight ground truth before any
  // agent reasoning (mock executor supplies canned preflight under MOCK_MODE).
  it("Test 14 - preflight runs info-first at run start", async () => {
    const { advance } = await import("../ai/orchestrator.js");
    const { createRun } = await import("../store/runs.js");
    const { getDb } = await import("../store/db.js");
    const { getAuditEvents } = await import("../store/audit.js");
    ...
    const run = createRun(1, "10.0.0.1:22");
    await advance(run.id); // first /next -> preflight + first proposal

    const obs = getDb()
      .all<{ source: string; content: string }>("SELECT * FROM observations WHERE run_id = ?", [
        run.id,
      ])
      .find((o) => o.source === "ssh" && o.content.startsWith("PREFLIGHT"));
    expect(obs).toBeDefined();
    expect(getAuditEvents(run.id).some((e) => e.type === "preflight.completed")).toBe(true);
  });

  // Safe reversible process: a fix whose COMMAND fails (non-zero exit) auto-stages
  // its rollback as the next human-approved step (not auto-executed).
  it("Test 15 - failed fix command auto-proposes its rollback for approval", async () => {
    const { advance } = await import("../ai/orchestrator.js");
    const { createRun } = await import("../store/runs.js");
    const { getDb } = await import("../store/db.js");
    const { getAuditEvents } = await import("../store/audit.js");
    ...
    const approveLatest = (cmd: string, exitCode: number) => {
      const pending = db
        .all<{ id: string; status: string }>("SELECT * FROM command_approvals WHERE run_id = ?", [
          run.id,
        ])
    ...
    await advance(run.id); // diagnostic proposed
    await approveLatest("systemctl status x", 0); // -> OBSERVING
    await advance(run.id); // -> PLANNING_FIX
    await advance(run.id); // -> WAITING (fix proposed, rollback_command persisted)
    const state = await approveLatest("sudo systemctl restart x", 1); // FIX command fails

    expect(state.phase).toBe("WAITING_FOR_APPROVAL");
    const pending = db
      .all<{ proposed_command: string; status: string }>(
        "SELECT * FROM command_approvals WHERE run_id = ?",
        [run.id],
      )
      .find((a) => a.status === "PENDING");
    ...
    const events = getAuditEvents(run.id);
    expect(events.some((e) => e.type === "fix.failed")).toBe(true);
    expect(events.some((e) => e.type === "fix.rollback_proposed")).toBe(true);
  });

  // A fix that runs cleanly but the validator says NOT_FIXED also stages a rollback.
  it("Test 16 - NOT_FIXED validation auto-proposes the rollback", async () => {
    const { advance } = await import("../ai/orchestrator.js");
    const { createRun } = await import("../store/runs.js");
    const { getDb } = await import("../store/db.js");
    type SshExecutor = import("../ssh/types.js").SshExecutor;
    validatorSpy.mockResolvedValueOnce({
      status: "NOT_FIXED",
      benefitCheck: "service still down",
      persistenceCheck: null,
    ...
    const approveLatest = (cmd: string) => {
      const pending = db
        .all<{ id: string; status: string }>("SELECT * FROM command_approvals WHERE run_id = ?", [
          run.id,
        ])
    ...
    await advance(run.id); // diagnostic
    await approveLatest("systemctl status x"); // -> OBSERVING
    await advance(run.id); // -> PLANNING_FIX
    await advance(run.id); // -> WAITING (fix)
    await approveLatest("sudo systemctl restart x"); // fix runs clean -> VALIDATING
    const state = await advance(run.id); // validator NOT_FIXED -> rollback proposed
