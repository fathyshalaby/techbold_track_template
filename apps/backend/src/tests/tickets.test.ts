...
beforeEach(async () => {
  ...
  const origStatuses: Record<number, "OPEN" | "PENDING" | "DONE"> = {
    1: "OPEN",
    2: "OPEN",
    3: "PENDING",
    4: "DONE",
...
describe("GET /api/tickets", () => {
  ...
  it("sorts by priority when sort=priority", async () => {
    ...
    for (let i = 1; i < body.length; i++) {
      expect(order[body[i].priority] ?? 99).toBeGreaterThanOrEqual(
        order[body[i - 1].priority] ?? 0,
      );
    }
  });
...
describe("Error mapping - PhoenixAuthError -> 502", () => {
  it("returns 502 when upstream throws PhoenixAuthError", async () => {
    ...
    const spy = vi
      .spyOn(mockModule.default.prototype, "listTickets")
      .mockRejectedValueOnce(new PhoenixAuthError("401 from upstream"));
    const res = await app.request("/api/tickets");
