...
export function makeJsonlAdapter(): DbAdapter {
  ...
  function extractWhere(
    sql: string,
    params: unknown[],
  ): { column: string; value: unknown } | undefined {
  ...
  return {
    ...
    run(sql: string, params: unknown[] = []): void {
      ...
      if (
        /^\s*(?:UPDATE|DELETE\s+FROM)\s+(audit_events|command_results|observations)\b/i.test(sql)
      ) {
      ...
      if (/^\s*INSERT/i.test(sql)) {
        ...
        let paramIdx = 0;
        let am: RegExpExecArray | null = assignRe.exec(setMatch[1]);
        while (am !== null) {
          const col = am[1];
          ...
          if (isCoalesce && (value === null || value === undefined)) {
            am = assignRe.exec(setMatch[1]);
            continue;
          }
...
export function getDb(dbPath?: string): DbAdapter {
  ...
  try {
    ...
    if (!anyMock) {
      throw new Error(
        `[store] SQLite unavailable (${reason}) and not in mock mode. The in-memory fallback is non-durable and would lose the audit trail. Refusing to start. Fix the native better-sqlite3 build or mount a writable data dir.`,
      );
    }
    console.warn(
      `[store] SQLite unavailable (${reason}). Falling back to an IN-MEMORY store (mock mode). Run state and the audit trail will NOT survive a restart.`,
    );
