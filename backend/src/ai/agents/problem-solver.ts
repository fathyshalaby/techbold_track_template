...
export const MOCK_FIX_PROPOSAL: FixProposal = {
  rootCause: 'status-api could not restart because port 8080 was already occupied by a stale process',
  command: 'sudo systemctl restart status-api',
  rationale: 'Restart the managed service after diagnostics identified the service as failed in mock mode',
