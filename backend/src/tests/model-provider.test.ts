...
import { parseEnv, effectiveLlmProvider } from '../env.js';
...
describe('effectiveLlmProvider', () => {
  it('defaults to openai', () => {
    expect(effectiveLlmProvider({ LLM_PROVIDER: 'openai', AZURE_ENDPOINT: '', LLM_BASE_URL: '' })).toBe(
      'openai',
    );
  });
  it('selects azure when an endpoint is present (even without explicit provider)', () => {
    expect(
      effectiveLlmProvider({ LLM_PROVIDER: 'openai', AZURE_ENDPOINT: 'https://x.azure.com', LLM_BASE_URL: '' }),
    ).toBe('azure');
  });
  it('selects openai-compatible when a base URL is present', () => {
    expect(
      effectiveLlmProvider({ LLM_PROVIDER: 'openrouter', AZURE_ENDPOINT: '', LLM_BASE_URL: 'https://or/api/v1' }),
    ).toBe('openai-compatible');
  });
  it('azure wins over a base URL when both are set', () => {
    expect(
      effectiveLlmProvider({ LLM_PROVIDER: '', AZURE_ENDPOINT: 'https://x.azure.com', LLM_BASE_URL: 'https://or' }),
    ).toBe('azure');
  });
});
...
describe('resolveModelConfig', () => {
  it('openai (default): no baseURL, model from LLM_MODEL', () => {
    const cfg = resolveModelConfig(parseEnv({ ...NO_PHX_SSH, OPENAI_API_KEY: 'k', LLM_MODEL: 'gpt-4o' }));
...
describe('env requires the selected provider credentials', () => {
  ...
  it('openai-compatible requires LLM_BASE_URL', () => {
    expect(() => parseEnv({ ...NO_PHX_SSH, LLM_PROVIDER: 'openai-compatible', OPENAI_API_KEY: 'k' })).toThrow(
      /LLM_BASE_URL/,
    );
  });
