...
export function getAuditEvents(runId: string): AuditEvent[] {
  const rows = getDb().all<AuditEvent>('SELECT * FROM audit_events WHERE run_id = ? ORDER BY ts ASC', [runId]);
...
export function createPendingApproval(
  ...
  return CommandApprovalSchema.parse(
    db.get('SELECT * FROM command_approvals WHERE id = ?', [id]),
  );
}
...
export function appendCommandResult(
  ...
  return CommandResultSchema.parse(
    db.get('SELECT * FROM command_results WHERE id = ?', [id]),
  );
}
...
export function saveActivityDraft(
  ...
  return ActivityDraftSchema.parse(
    db.get('SELECT * FROM activity_drafts WHERE id = ?', [id]),
  );
}
